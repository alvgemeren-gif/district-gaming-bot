...
const data = new SlashCommandBuilder()
	...
	.setDescription('Maak een reaction role panel met knoppen.')
	.addStringOption(option =>
		option
			.setName('titel')
			.setDescription('Titel van het panel.')
			.setRequired(true)
	)
	.addStringOption(option =>
		option
...
for (let index = 1; index <= MAX_ROLES; index += 1) {
	data.addRoleOption(option =>
		option
			.setName(`rol${index}`)
			.setDescription(`Rol ${index} voor dit panel.`)
			.setRequired(index === 1)
	);
}
...
module.exports = {
	...
	async execute(interaction) {
		...
		await interaction.reply({
			content: 'Reaction role panel aangemaakt.',
			ephemeral: true,
		});
	},
